...
struct ThemeTests {
    ...
    @Test func oledDarkModeUsesTrueBlack() {
        let regularDark = JournalTheme(isDark: true, useOLED: false)
        let oledDark = JournalTheme(isDark: true, useOLED: true)
        // OLED mode should use true black
        #expect(oledDark.bg == .black)
        // Regular dark mode should not use true black
        #expect(regularDark.bg != .black)
    }
    
    @Test func oledModeOnlyAppliesInDarkMode() {
        let lightWithOLED = JournalTheme(isDark: false, useOLED: true)
        let lightWithoutOLED = JournalTheme(isDark: false, useOLED: false)
        // OLED setting should have no effect in light mode
        #expect(lightWithOLED.bg == lightWithoutOLED.bg)
    }
    ...
    @Test func oledThemeHasAllRequiredProperties() {
